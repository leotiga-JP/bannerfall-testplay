# Bannerfall Phase 3.10.1 — 戦闘教義拡張 Hotfix

## 3.10.1 Hotfix
- ロビーの兵科カードを12兵科すべて同じ定義から動的生成し、狙撃兵・工兵・胸甲騎兵を含めて確実に選択できるよう修正。
- 次回兵科選択画面に残っていた旧兵科カードの重複HTMLを削除。
- 擲弾兵の `4` 手榴弾を再修正。整列途中でも使用でき、カーソルが近すぎる場合は部隊正面へ投げます。
- 馬防柵は `5` で設置モード → 設置位置を左クリックする二段階操作へ変更。遠すぎる位置をクリックしても設置可能距離へ自動補正します。
- 工兵への兵科変更・リスポーン時にローカル操作状態を再同期し、旧兵科の武器/突撃状態が残って移動を阻害しないよう修正。
- 斧装備中に敵の馬防柵を左クリックすると直接破壊できます。通常歩兵でも大ダメージ、工兵はさらに高い破壊力を持ちます。
...
- `1 / 2 / 3` — 歩兵のマスケット / 銃剣 / 斧（斧で敵馬防柵を左クリックすると高ダメージ）
- `4` — 擲弾兵: 手榴弾
- `5` — 馬防柵設置モード（所持兵科のみ）→ 左クリックで設置
- `右クリック長押し → 離す` — 突撃
- `F` — 再整列 / 白兵戦離脱
- `Space` — 自部隊へカメラ追従
- `N` — 次回兵科予約画面
